...
impl From<Row> for DbStationItem {
    fn from(mut row: Row) -> Self {
        DbStationItem {
            ...
            lastchangetime_iso8601:      row.take_opt("Creation").transpose().ok().flatten().map(|x|chrono::DateTime::<chrono::Utc>::from_utc(x, chrono::Utc)),
            ...
            lastchecktime_iso8601:       row.take_opt("LastCheckTime").transpose().ok().flatten().map(|x|chrono::DateTime::<chrono::Utc>::from_utc(x, chrono::Utc)),
            lastchecktime:               row.take_opt("LastCheckTimeFormated").unwrap_or(Ok("".to_string())).unwrap_or("".to_string()),
            lastcheckoktime_iso8601:     row.take_opt("LastCheckOkTime").transpose().ok().flatten().map(|x|chrono::DateTime::<chrono::Utc>::from_utc(x, chrono::Utc)),
            lastcheckoktime:             row.take_opt("LastCheckOkTimeFormated").unwrap_or(Ok("".to_string())).unwrap_or("".to_string()),
            lastlocalchecktime_iso8601:  row.take_opt("LastLocalCheckTime").transpose().ok().flatten().map(|x|chrono::DateTime::<chrono::Utc>::from_utc(x, chrono::Utc)),
            lastlocalchecktime:          row.take_opt("LastLocalCheckTimeFormated").unwrap_or(Ok("".to_string())).unwrap_or("".to_string()),
            clicktimestamp_iso8601:      row.take_opt("ClickTimestamp").transpose().ok().flatten().map(|x|chrono::DateTime::<chrono::Utc>::from_utc(x, chrono::Utc)),
