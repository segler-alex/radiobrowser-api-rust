...
WHERE
    StationUuid=:StationUuid"#,
        params!{
            ...
            "Tags" => &station.tags,
            "Votes" => station.votes,
            "Subcountry" => &station.state,
            "clickcount" => station.clickcount,
            "ClickTrend" => station.clicktrend,
            "Codec" => &station.codec,
            "LastCheckOK" => station.lastcheckok,
            "Bitrate" => station.bitrate,
            "UrlCache" => &station.url_resolved,
            "Hls" => station.hls,
            ...
            "SslError" => station.ssl_error,
            "LanguageCodes" => &station.languagecodes,
            "ExtendedInfo" => station.has_extended_info,
    ...
    */
/*
    fn update_station_timestamps(&self, station: &DbStationItem, reason: &str) -> Result<(), Box<dyn Error>> {
        let mut transaction = self.pool.start_transaction(TxOpts::default())?;
        transaction.exec_drop(
r#"UPDATE Station SET
    Creation=UTC_TIMESTAMP(),
    ClickTimestamp=UTC_TIMESTAMP(),
    LastCheckTime=UTC_TIMESTAMP(),
    LastCheckOkTime=UTC_TIMESTAMP(),
    LastLocalCheckTime=UTC_TIMESTAMP()
WHERE
    StationUuid=:stationuuid"#,
params!{
            "stationuuid" => &station.stationuuid,
